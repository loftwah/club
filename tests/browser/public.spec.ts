...
test.describe("public routes (semantic HTML, no essential JS)", () => {
  test("GET / renders the homepage", async ({ page }) => {
    ...
    await expect(page.getByRole("heading", { level: 1 })).toContainText(/commitment|cancel/i);
  ...
  // Issue #4: waitlist hero CTA must use the canonical --calendar
  // token for its interactive state. The previous CSS used an
  // undefined --cobalt custom property, which produced an invalid
  // computed background. This regression asserts the computed
  // background changes on hover and that keyboard focus-visible
  // applies a deliberate treatment without losing a visible focus
  // indicator.
  test("waitlist CTA hover resolves to the canonical --calendar token", async ({ page }) => {
    await page.goto("/waiting-list/");
    const cta = page.locator(".pwy-hero__cta");
    ...
    const normal = await readBackground();
    // Hover via Playwright to apply :hover.
    await cta.hover();
    const hovered = await readBackground();
    expect(hovered).not.toBe(normal);
    // Canonical schedule cobalt (--calendar: #2447ff) is rgb(36, 71, 255).
    expect(hovered).toBe("rgb(36, 71, 255)");

    // Move the mouse away and tab to the CTA so :focus-visible applies.
    await page.mouse.move(0, 0);
    await page.keyboard.press("Tab");
    // The first focusable on /waiting-list/ is the skip link, then the
    // nav, then the hero CTA. Tab until the CTA is focused.
    for (let i = 0; i < 20; i += 1) {
      const focusedClass = await page.evaluate(() => document.activeElement?.className ?? "");
      if (focusedClass.includes("pwy-hero__cta")) break;
      await page.keyboard.press("Tab");
    }
    const focusedClass = await page.evaluate(() => document.activeElement?.className ?? "");
    expect(focusedClass).toContain("pwy-hero__cta");

    const focused = await readBackground();
    expect(focused).toBe("rgb(36, 71, 255)");

    // Focus-visible must keep a visible focus indicator (outline).
  ...
  test("GET /how-it-works renders the canonical arc", async ({ page }) => {
    ...
    expect(response?.status()).toBe(200);
    await expect(page.getByText(/I was on the list|A real commitment/)).toBeVisible();
...
test.describe("reduced motion", () => {
  test("prefers-reduced-motion: reduce still surfaces all essential content", async ({
    ...
    // are suppressed.
    await expect(page.getByRole("heading", { level: 1 })).toContainText(/commitment|cancel/);
    await expect(page.getByText(/You were on the list/i)).toBeVisible();
...
test.describe("no-JavaScript fallback path", () => {
  test("the home page does not require JavaScript for essential content", async ({ browser }) => {
    ...
    expect(html).toContain("A real commitment");
